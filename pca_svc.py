...
def test():
    ...
    svc = joblib.load(workPath+'Model/SVM/pca_'+str(components)+'_svm.m')
    x_train_reduce=ss.fit_transform(x_train_reduce)
    x_test_reduce = pca.transform(x_test)
    x_test_reduce = ss.transform(x_test_reduce)
    print 'PREDICT>>>>>>'
    y_predict=svc.predict(x_test_reduce)
    print 'PREDICT_END>>>>>>'
    scores = cross_validation.cross_val_score(svc, x_train_reduce, y_train, cv=10)
    test_ac=svc.score(x_test_reduce,y_test)
    cross_ac=scores.mean()
    cross_er=scores.std()*2
    out_str='The Accuracy of SVC_PCA'+str(components)+' is '+str(test_ac)+"\n"
    out_str=out_str+str(classification_report(y_test,y_predict,target_names=['0','1']))+"\n"
    with open(workPath+'Model/SVM/SVM_PCA_Report.txt','a+') as file:
        file.write(out_str)
    print 'The Accuracy of SVC_PCA'+str(components)+' is ', test_ac
    #target_name=np.mat([0,1]).T
    print classification_report(y_test,y_predict,target_names=['0','1'])
    with open(workPath+'Model/SVM/Cross_Ac.txt','a+') as file:
   ...
   # test()
    for i in range(100,3100,100):
       ...
       # print scores
        ...
        x_train_reduce=ss.fit_transform(x_train_reduce)
        print y_train
        svc.fit(X=x_train_reduce,y=y_train)
        print 'SAVEING>>>>>>'
        joblib.dump(svc,workPath+'Model/SVM/pca_'+str(components)+'_svm.m')
        print 'END>>>>>> '
        test()
